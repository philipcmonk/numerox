...
# A simple script to test the installed version of numerox by calling
# 'numerox.test()'. Key features:
#   -- convenient command-line syntax
#   -- sets exit status appropriately, useful for automated test environments

# It would be better to set this up as a module in the numerox namespace, so
...
# TODO: numerox doesn't support python 2.4

# In case we are run from the source directory, we don't want to import
# numerox from there, we want to import the installed version:
sys.path.pop(0)
...
result = numerox.test(options.mode,
